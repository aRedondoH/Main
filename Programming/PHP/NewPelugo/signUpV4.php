...
$message = "Gracias por confiar en PeluGo, ahora todo el mundo podrá encontrar su peluquería de forma sencilla, además si nos envia la foto de su peluqueria junto con su logo, la ponemos en portada. Del mismo modo, si dispone de tutoriales o eventos que quiera publicar, PeluGo se los publica de forma GRATUITA, puede responder directamente a este email si lo desea\n
...
Correo: " .$emailToSafe. "\n
Teléfono: " .$phoneToSafe. "\n
si necesita modificar sus datos o le surge cualquier duda, no dude en ponerse en contacto con nosotros a través de info@pelugo.es ";;;;
$from = "info@pelugo.es";;;;
$headers = "From:" .$from;
// Send email
mail($to,$subject,$message,$headers);
...
$timetableToSafe = $_POST['timeTable'];
$timetableToSafe = strtr($timetableToSafe, $unwanted_array);
}
if (!empty($_POST['email'])){
$emailToSafe = $_POST['email'];
$emailToSafe = strtr($emailToSafe, $unwanted_array);
}
if (!empty($_POST['phoneNumber'])){
$phoneToSafe = $_POST['phoneNumber'];
$phoneToSafe = preg_replace('/\s+/','',$phoneToSafe);
}
if (!empty($_POST['map'])){
$mapToShow = $_POST['map'];
}
