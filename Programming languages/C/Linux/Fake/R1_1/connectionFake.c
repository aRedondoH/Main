...
int transferEventFake(int agentId, int eventTypeId, char *description) {
	...
	ssl_set_dbg(&ssl, my_debugFake, stdout);
...
int transferEventFakeWrongAgentCert(int agentId, int eventTypeId, char *description) {
	...
	x509_crt_init(&ssl_agent_crt);
	if ((ret = x509_crt_parse_file(&ssl_agent_crt, SSL_AGENT_RSA_WRONG_CERT)) != 0) {
		printf("x509parse_crtfile AGENT CERT returned %d\n", ret);
		goto exit_transferEvent;
	}
...
int transferEventFakeWrongAgentKey(int agentId, int eventTypeId, char *description) {
	...
	pk_init(&ssl_agent_key);
	if ((ret = pk_parse_keyfile(&ssl_agent_key, SSL_AGENT_RSA_WRONG_KEY, NULL ))
			!= 0) {
		printf("pk_parse_keyfile AGENT KEY returned %d\n\n", ret);
		goto exit_transferEvent;
	}
...
int transferEventFakeWrongDetectorCert(int agentId, int eventTypeId, char *description) {
	...
	x509_crt_init(&ssl_ca_crt);
	if ((ret = x509_crt_parse_file(&ssl_ca_crt, DETECTOR_CA_WRONG_CERT)) != 0) {
		printf("x509parse_crtfile CA CERT returned %d\n", ret);
		goto exit_transferEvent;
	}
