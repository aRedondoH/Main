...
/* list of struct */
struct process_info listOfProcessesOfStruct[MAXNUMBERPROCESSES];
...
void printArrayOfStruct (struct process_info pepe[MAXNUMBERPROCESSES]){
	int f;
	/* print list of processes */
	for (f=0;f<=numberOfProcesses;f++){
		printf("Process: %s, pid: %d, pos: %d\n",pepe[f].name, pepe[f].processPID, f);
	}
	printf("\n");
	fflush(stdout);
}
...
void makeAndPrintListOfEveryProcessRunningInListOfRegister() {
	char line[MAXLENGHTLINE];
	FILE *fp;
	fp = popen("ps aux | awk '{printf $11 \" \" $2 \"\\n\"}' | tail -n +2 ",
			"r");
	...
	// manually
	/*listOfProcessesOfStruct[0].name=malloc(strlen("Paco") + 1);
	 strcpy(listOfProcessesOfStruct[0].name,"Paco");
	 listOfProcessesOfStruct[0].processPID=90;
	 printf("process %s ", listOfProcessesOfStruct[0].name);
	 printf("pid %d\n", listOfProcessesOfStruct[0].processPID);*/
	...
	while (fgets(line, sizeof line, fp) != NULL ) {
		...
		// insert process into array
		currentPosition++;
		// Memory allocation for each process name
		listOfProcessesOfStruct[currentPosition].name = malloc(strlen(line) + 1);
		// copy the process into array
		strcpy(listOfProcessesOfStruct[currentPosition].name, token1);

		if (token1 != NULL ) {
			// Incrementing pointer to the next token
			token1 = strtok(NULL, " ");
			// Insert PID into struct
			listOfProcessesOfStruct[currentPosition].processPID = atoi(token1);
		}
	...
	printArrayOfStruct(listOfProcessesOfStruct);
